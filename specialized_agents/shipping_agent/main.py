...
async def lifespan(app: FastAPI):
    ...
    logger.info("배송 정보 에이전트 시작 중...")
    
    # 에이전트 객체 생성
    agent = ShippingAgent()
    await agent.startup()
    
    logger.info("배송 정보 에이전트가 http://localhost:8002에서 실행 중입니다")
    
    yield
    
    # 종료 시 처리 (필요한 경우)
    logger.info("배송 정보 에이전트를 종료합니다...")


app = FastAPI(
    title="배송 정보 에이전트",
    description="배송 상태 및 정보를 제공하는 A2A 프로토콜 호환 에이전트",
    version="1.0.0",
    lifespan=lifespan
)
